/**
 * logs.js — device log lines shipped over the uplink.
 *
...
async function loadDay() {
  ...
  try {
    rows = await fetchLogs(deviceId, {
      from_ts: dayStart, to_ts: dayStart + DAY, min_level: minLevel, limit: 5000,
    });
  } catch (e) {
...
async function refresh() {
  ...
  try {
    fresh = await fetchLogs(currentDevice(), {
      from_ts: last + 0.001, to_ts: dayStart + DAY, min_level: minLevel, limit: 5000,
    });
  } catch (e) {
    return;
  }
...
function render(jumpToEnd) {
  ...
  tbody.innerHTML = rows.map(r => `
    <tr class="lvl-${r.level}">
      ...
      <td class="log-mod">${esc(r.module)}</td>
